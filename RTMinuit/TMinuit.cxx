//TMinuit Originally From ROOT https://root.cern.ch
//Decouple from ROOT by Piti Ongmongkolkul
...
//______________________________________________________________________________
TMinuit::TMinuit(const TMinuit &minuit)
{
...
//______________________________________________________________________________
void InteractiveFCNm(int &npar, double *gin, double &f, double *u, int flag)
{
...
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
   ...
   if (funcname) {
      ...
      fMethodCall->InitWithPrototype(funcname,"int&,double*,double&,double*,int");
   }
...
//*-*-                      make sure starting at the right place
      ...
      if (fs1 != fAmin) {
         df    = fAmin - fs1;
         mnwarn("D", "MNDERI", string::Format("function value differs from AMIN by %12.3g",df));
         fAmin = fs1;
      }
...
//*-*-                          end of ICYC loop. too many iterations
      if (ncyc == 1) goto L50;
      mnwarn("D", "MNDERI", string::Format("First derivative not converged. %g%g",fGrd[i-1],grbfor));
L50:
      ;
   }
   mninex(fX);
   ...
   if (fISW[4] >= 2) {
      for (i = 1; i <= npard; ++i) {
         ...
         for (k = 1; nperln < 0 ? k >= iz : k <= iz; k += nperln) {
            ...
            if (k2 > iz) k2 = iz;
            for (kk = k; kk <= k2; ++kk) {
               ctemp += string::Format("%10.3e ",emat[i + kk*emat_dim1]);
            }
...
//*-*-          sure that WORD7(1)=0 if LLIST=0
   ...
   if (fCword(0,7) != "SET PRI" || fWord7[0] >= 0) {
      if (fISW[4] >= 0) {
         ...
         for (i = 1; i <= lnow; ++i) {
            ctemp += string::Format("%12.4g",plist[i-1]);
         }
         Printf("%s",(const char*)ctemp);
         inonde = 0;
         if (llist > lnow) {
...
//*-*-                make sure starting at the right place
   ...
   if (fs1 != fAmin) {
      df    = fAmin - fs1;
      mnwarn("D", "MNHESS", string::Format("function value differs from AMIN by %g",df));
   }
   fAmin = fs1;
   if (ldebug) {
...
L26:
         mnwarn("W", "HESSE", string::Format("Second derivative zero for parameter%d",iext));
         goto L390;
//*-*-                            SAG is not zero
L30:
         g2bfor    = fG2[i-1];
...
//*-*-                      end of step size loop
      if (!skip50)
         mnwarn("D", "MNHESS", string::Format("Second Deriv. SAG,AIM= %d%g%g",iext,sag,aimsag));
...
//*-*-                                      loop satisfied = too many iter
      mnwarn("D", "MNHES1", string::Format("Too many iterations on D1.%g%g",grdold,grdnew));
L60:
...
//*-*-    NCOEF is number of coeff. that fit on one line, not to exceed 20
   ...
   for (id = 1; id <= nparm; ++id) {
      ctemp += string::Format(" %6d",fNexofi[id-1]);
   }
   Printf("%s",(const char*)ctemp);
   for (i = 1; i <= fNpar; ++i) {
      ix  = fNexofi[i-1];
      ...
      for (it = 1; it <= nparm; ++it) {
         ctemp += string::Format(" %6.3f",fMATUvline[it-1]);
      }
      Printf("%s",(const char*)ctemp);
      if (i <= nparm) continue;
      ctemp = "                   ";
      for (iso = 1; iso <= 10; ++iso) {
         ...
         for (it = nsofar + 1; it <= nparm; ++it) {
            ctemp = ctemp + string::Format(" %6.3f",fMATUvline[it-1]);
         }
         Printf("%s",(const char*)ctemp);
         if (i <= nparm) break;
      }
...
//*-*-                            . . request for another variable parameter
   ...
   if (b - a > 1e7) {
      mnwarn("W", "PARAM DEF", string::Format("LIMITS ON PARAM%d TOO FAR APART.",k));
      if (fLwarn) fLphead = kTRUE;
   }
   danger = (b - uk)*(uk - a);
   if (danger < 0) {
...
//*-* --                          there are two limits
      ...
      if (yy2 >= 1 - fEpsma2) {
         ...
         mnwarn("W", fCfrom, string::Format("VARIABLE%d%s",i,chbuf2.Data()));
      } else {
         pinti = TMath::ASin(yy);
      }
   }
...
//______________________________________________________________________________
void TMinuit::mnpout(int iuext1, string &chnam, double &val, double &err, double &xlolim, double &xuplim, int &iuint) const
{
...
//*-*-                       Check if negative or zero on diagonal
   for (i = 1; i <= fNpar; ++i) {
      ndex = i*(i + 1) / 2;
      if (fVhmat[ndex-1] <= 0) {
         mnwarn("W", fCfrom, string::Format("Negative diagonal element %d in Error Matrix",i));
      }
      if (fVhmat[ndex-1] < dgmin) dgmin = fVhmat[ndex-1];
   }
   if (dgmin <= 0) {
      dg    = epspdf + 1 - dgmin;
      mnwarn("W", fCfrom, string::Format("%g added to diagonal of error matrix",dg));
   } else {
      dg = 0;
   }
...
//______________________________________________________________________________
Bool_t TMinuit::mnunpt(string &cfname)
{
